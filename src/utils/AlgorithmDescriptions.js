  ...
  'Quick Sort': {
    description:
      <>
        ...
        <Typography variant="body1" gutterBottom pt={1} fontSize={'1rem'}>
          Quick sort's partitioning may be done in two different schemes: Hoare's and Lomuto's. Hoare's scheme uses two pointers starting from both ends of the array and moving towards each other until an inversion is found, and then swaps the elements. It tends to be faster and uses fewer swaps compared to Lomuto's. On the other hand, Lomuto's scheme works by choosing a pivot and iterating through the array, rearranging elements such that those less than the pivot come before those greater than the pivot, then finally swapping the pivot into its correct position. While simpler to understand and implement, it generally performs more swaps than Hoare's method. This visualization uses Lomuto's scheme.
        </Typography>
        <Typography variant="body1" gutterBottom pt={1} fontSize={'1rem'}>
          The efficiency of Quick Sort comes from the fact that, on average, each element only needs to be compared about log(n) times. This allows Quick Sort to sort n elements in O(n log n) time, making it much faster than other simple algorithms like Bubble Sort or Insertion Sort for large lists. However, in the worst case, Quick Sort can take up to O(n²) time, but this scenario is rare.
        </Typography>
        <Typography variant="body1" gutterBottom pt={1} fontSize={'1rem'}>
